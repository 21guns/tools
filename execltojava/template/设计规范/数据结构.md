...
### 案件推送记录表 case_push_record

| 中文名称     | 字段名             | 类型           | 主键 | 允许为空 | 是否唯一 | 默认值 | 备注             |
|--------------|--------------------|----------------|------|----------|----------|--------|------------------|
| ID           | `id`               | `BIGINT(19)`   | Y    | N        | Y        |        |                  |
| 案件ID       | `case_id`          | `BIGINT(19)`   | N    | N        | N        |        |                  |
| 案件阶段     | `case_status`      | `TINYINT(4)`   | N    | N        | N        |        |                  |
| 审理结果     | `trial_result`     | `VARCHAR(45)`  | N    | N        | Y        |        |                  |
| 机构名称     | `institution_name` | `VARCHAR(45)`  | N    | N        | N        |        |                  |
| 联系人       | `receiver_name`    | `VARCHAR(45)`  | N    | N        | N        |        |                  |
| 联系电话     | `receiver_phone`   | `VARCHAR(45)`  | N    | N        | N        |        |                  |
| 是否短信通知 | `msg_notice_flag`  | `TINYINT(4)`   | N    | N        | N        |        | `1`: 是，`2`: 否 |
| 邮箱         | `receiver_email`   | `VARCHAR(45)`  | N    | N        | N        |        |                  |
| 创建时间     | `gmt_created`      | `DATETIME(3)`  | N    | N        | N        |        |                  |
| 更新时间     | `gmt_modified`     | `DATETIME(3)`  | N    | N        | N        |        |                  |
| 备注         | `note`             | `VARCHAR(255)` | N    | Y        | N        |        |                  |
